...
public class StudentApi {
	...
	public void StudentInsertApi(Locale locale, Model model, HttpServletResponse response) {
		...
		if(profno == 0) {web.printJsonRt("학생번호를 입력하세요."); return;}
		
		// 파라미터 Beans 구성
		// --> study.spring.hellospring.model.Student;
		Student student = new Student();
	...
	/** 학생 수정 API */
	@ResponseBody
	@RequestMapping(value = "student_api/StudentUpdateApi", method = RequestMethod.POST)
	public void StudentUpdateApi(Locale locale, Model model, HttpServletResponse response) {
